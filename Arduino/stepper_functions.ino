...
// Updated Stepper functions - not midi compatible?? 
//#include <Stepper.h>
//#define stepsPerRevolution 5000  // actual = 200 (microstep) 
...
// Move rows, array or positions up to 100
// @param positions - array of row positions 
// @returns CURR_ROW - the current row
int moveRows(uint8_t positions[100]) {
  for (byte i = 0; i < sizeof(positions); i = i + 1) {
...
// Move columns, array or positions up to 100
// @param positions - array of column positions 
// @returns CURR_COL - the current column
int moveCols(uint8_t positions[100]) {
  for (byte i = 0; i < sizeof(positions); i = i + 1) {
